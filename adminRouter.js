...
router.get("/game", authMiddleware, requireRole("admin"), async (_req, res) => {
  try {
    ...
    return res.json({
      ...
      central,
      persistenceNote:
        "เกมส่วนกลาง: สถานะรอบเกมถูกเก็บในฐานข้อมูล — หลังรีสตาร์ท API ผู้เล่นที่ยังมี sessionId + สิทธิ์รอบเกมยังเล่นต่อได้ (ไม่หักหัวใจซ้ำ) · เกม legacy ในเครื่องยังเป็นแค่หน่วยความจำ · ตั้งค่าเกมส่วนกลางในแท็บ「เกมส่วนกลาง」"
    });
  } catch (e) {
    return res.status(500).json({ ok: false, error: e.message });
  }
