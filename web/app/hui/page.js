...
export const metadata = {
  title: "แพลตฟอร์มหัวใจ (เวอร์ชันเดิม) | HUAJAIY",
  description:
    "หน้าแรกแบบเดิมของ HUAJAIY — เกม ร้านค้า หัวใจ · หน้าหลักใหม่คือ Workspace (TailAdmin) ที่ /"
};

export default async function HuiHomePage() {
  ...
  return (
    <>
      ...
      <main className="mx-auto w-full max-w-5xl px-4 pb-20 pt-8 md:pt-12">
        <p className="mb-6 rounded-2xl border border-hui-border bg-hui-pageTop/90 px-4 py-3 text-sm text-hui-body">
          นี่คือ<strong>หน้าแรกแบบเดิม</strong>ของแพลตฟอร์ม — หน้าแรกของโดเมนตอนนี้คือ{" "}
          <Link href="/" className="font-semibold text-hui-section underline">
            Workspace (TailAdmin)
          </Link>
        </p>
        ...
        <section className="mt-14 md:mt-16" aria-labelledby="game-published-heading">
          ...
          {featuredGames.length > 0 ? (
            <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {featuredGames.map((g) => {
                const cover =
                  String(g.gameCoverUrl || "").trim() || DEFAULT_CENTRAL_GAME_COVER_PATH;
                return (
                  <li key={g.id}>
                    <Link
