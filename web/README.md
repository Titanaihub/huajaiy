...
### แก้ error: `There is a problem with the server configuration.`

1. ใน Render ต้องมี **`NEXTAUTH_SECRET`** (หรือ **`AUTH_SECRET`**) ยาวอย่างน้อย ~32 ตัว — ถ้าไม่มีจะ error ตอน callback จาก Facebook
2. ต้องมี **`NEXTAUTH_URL`** = `https://huajaiy-web.onrender.com` (ไม่มี `/` ท้าย) ตรงกับ URL ที่ผู้ใช้เข้า
3. Save → **Manual Deploy** ใหม่
4. ชั่วคราว: ตั้ง **`NEXTAUTH_DEBUG=1`** แล้วดู **Logs** ใน Render
